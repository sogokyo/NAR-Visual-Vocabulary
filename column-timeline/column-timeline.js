function columnChart(data, stylename, media,yMin,yMax,yAxisHighlight, chartpadding,legAlign,labels, numTicksy, yAlign,interval,minAxis, ticks){
    ...
    yLabel
        .attr("transform",function(){
            ...
            else return "translate("+yLabelOffset+margin.left+","+margin.top+")"
            })

    yLabel.selectAll('text')
        .attr("style", null)
        .attr("dx","-.3em")
        // .attr("x",yticksize-(yLabelOffset-"g"))
    ...
    //specify the tick format - michael
    xLabel.text(function(d,i){
        //console.log(d.getMonth())//get specific information about the date for this tick

        //define the date formats we need to use
        var initialTickFormat = d3.time.format("%Y");
        var newYearFormat = d3.time.format("’%y");
        var regTickFormat = d3.time.format("%b.");
        var justMonth = d3.time.format("%B");

        //boolean testing of the date for the tick
        if (i==0){//first tick
            return initialTickFormat(d);
        }   else    {
            if (d.getMonth()==0){//January
                return newYearFormat(d);
            }   else{//any other month
                if (justMonth(d).length>3){
