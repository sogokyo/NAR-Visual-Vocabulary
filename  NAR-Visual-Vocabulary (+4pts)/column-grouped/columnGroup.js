function columnChart(data,stylename,media,plotpadding,legAlign, logScale, logScaleStart, yMin,yMax,yHighlight, numTicksy, ticks, yAlign){
    ...
    .rangeBands([5, plotWidth-yLabelOffset], .13);
...
function forecast(element){
    return element.annotate=="Forecast"
}
...
//add annotation
    ...
    d3.selectAll("." + media +"xAxis .tick text")
        .classed("forecast", function(d){ return x0.domain().indexOf(d) >= x0.domain().indexOf(annotations.filter(function(f){return f.annotate == "Forecast"})[0].cat)})
