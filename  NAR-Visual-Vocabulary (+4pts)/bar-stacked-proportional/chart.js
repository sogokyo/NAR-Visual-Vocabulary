...
function makeChart(data,seriesNames,stylename,media,plotpadding,legAlign,yAlign){
    ...
    key.selectAll("text")
        ...
        .attr("y","-.09em")
        .attr("x",function(d,i){
            return labelMarginL+(i*(plotWidth/4)+12)
        })
        .text(function(d){
   ...
   xStack(xData)
    ...
    var vOffset = (d3.select("#key").node().getBBox().height)+(d3.select("#key").node().getBBox().y)+27
    ...
    var groups = plot.append("g").attr("id","groups")
        ...
        .attr("transform",function(d,i){
            ...
            var offset = i*2.5;//creates spacing between rows
    ...
    groups.append("text")
        ...
        .attr("text-anchor","end")
        .attr("y",function(d){
            return yScale(d[0].height)/2+4.3
        })
