function columnChart(data,stylename,media,plotpadding,legAlign, logScale, logScaleStart, yMin,yMax,yHighlight, numTicksy, ticks, yAlign){
    ...
    .rangeBands([10, plotWidth-10-yLabelOffset], .12);
...
function forecast(element){
    return element.annotate=="Projection"
}
...
//add annotation
    ...
    d3.selectAll("." + media +"xAxis .tick text")
        .classed("forecast", function(d){ return x0.domain().indexOf(d) >= x0.domain().indexOf(annotations.filter(function(f){return f.annotate == "Projection"})[0].cat)})
